...
public class MemberDao {
	...
	public Member loginMember(Connection con, String id, String passwd) {
		...
		try {
			...
			common.jdbcDriver.JDBCTemplate.close(rset);
			common.jdbcDriver.JDBCTemplate.close(pstmt);
			// 앞에 (common.jdbcDriver.JDBCTemplate.)이 부분 만큼이 생략된 것이다.
			// 근데 이것을 매번 작성할 수는 없으니 (common.jdbcDriver.JDBCTemplate.) 부분을
			// import로 빼버린 것이다. + (.*)
			// (.*) --> 전부 다를 의미
		}
