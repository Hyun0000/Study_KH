...
import first.member.model.service.MembetService;
...
public class MemberSelectServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 이때 톰캣이 doGet() method를 호출시킨다. 호출의 결과는 response를 통해 client에게 돌려준다.
		// (쉽게말해서 client가 브라우저에서 url을 땅 치면 톰캣이 doGet() method를 호출하는 것이다.)
		// WEB이 된 이상 Run Class와 main()은 더 이상 필요없다.
		
		PrintWriter out = response.getWriter();
				
		ArrayList<MemberVo> volist = new MembetService().selectList();
